...
select indice_faltante from (values ('ux_grados_nombre_normalizado'),('ux_jornadas_nombre_normalizado'),('ux_secciones_contexto_nombre')) esperado(indice_faltante)
where not exists(select 1 from pg_indexes i where i.schemaname='public' and i.indexname=esperado.indice_faltante);

select indice_legacy from (values ('ux_secciones_contexto_jornada_nombre'),('ux_secciones_contexto_sin_jornada_nombre')) esperado(indice_legacy)
where exists(select 1 from pg_indexes i where i.schemaname='public' and i.indexname=esperado.indice_legacy);

select 'Indice de secciones incorrecto' error where not exists(
 select 1 from pg_indexes i where i.schemaname='public' and i.indexname='ux_secciones_contexto_nombre'
 and lower(i.indexdef) like '%unique index%'
 and lower(i.indexdef) like '%institucion_id%'
 and lower(i.indexdef) like '%ciclo_id%'
 and lower(i.indexdef) like '%grado_id%'
 and lower(i.indexdef) like '%lower(btrim(nombre))%'
 and lower(i.indexdef) not like '%jornada_id%');
