...
-- Configuracion del centro educativo y sus identificadores.
...
as $$
declare
  v_multi boolean; v_institucion_id uuid; v_activas integer;
...
begin
  ...
  if v_multi then
    if p_institucion_id is null then
      raise exception 'INSTITUTION_CONTEXT_REQUIRED: seleccione una institucion.' using errcode = 'SM003';
    end if;
  ...
  select * into v_institucion from public.instituciones
  where id = v_institucion_id and activo;
  if not found then
    raise exception 'La institucion no existe o esta inactiva.' using errcode = 'P0002';
  end if;
  select * into v_identificadores from public.configuracion_identificadores
  where institucion_id = v_institucion_id;
  return jsonb_build_object(
    'multiplesInstituciones', v_multi,
    'institucion', jsonb_build_object(
      'id', v_institucion.id, 'nombre', v_institucion.nombre,
      'nombreCorto', v_institucion.nombre_corto, 'direccion', v_institucion.direccion,
      'telefono', v_institucion.telefono, 'correo', v_institucion.correo,
    ...
    'identificadores', jsonb_build_object(
      ...
      'tiposIdentificacionPermitidos', coalesce(v_identificadores.tipos_identificacion_permitidos,
        array['identidad', 'pasaporte', 'otro']::text[])));
end;
$$;

create or replace function public.rpc_crear_institucion(
  p_nombre text, p_nombre_corto text default null, p_direccion text default null,
  p_telefono text default null, p_correo text default null, p_logo_url text default null,
...
set search_path = pg_catalog, public, pg_temp
as $$
declare v_multi boolean; v_institucion_id uuid; v_tipos text[];
begin
  if public.usuario_actual_id() is null
  ...
  if not v_multi and exists (select 1 from public.instituciones where activo) then
    raise exception 'ACTIVE_INSTITUTION_ALREADY_EXISTS: el modo single ya tiene una institucion activa.' using errcode = 'SM004';
  end if;
  select coalesce(array_agg(distinct lower(btrim(tipo)) order by lower(btrim(tipo))), '{}')
  into v_tipos from unnest(coalesce(p_tipos_identificacion_permitidos, '{}'::text[])) tipo
  where btrim(tipo) <> '';
  insert into public.instituciones (nombre, nombre_corto, direccion, telefono, correo, logo_url)
  values (btrim(p_nombre), nullif(btrim(p_nombre_corto), ''), nullif(btrim(p_direccion), ''),
    nullif(btrim(p_telefono), ''), nullif(lower(btrim(p_correo)), ''), nullif(btrim(p_logo_url), ''))
  ...
  values (v_institucion_id, p_rne_requerido, p_identificacion_civil_requerida,
...
create or replace function public.rpc_actualizar_institucion(
  p_institucion_id uuid, p_nombre text, p_nombre_corto text default null,
  p_direccion text default null, p_telefono text default null, p_correo text default null,
  p_logo_url text default null, p_rne_requerido boolean default false,
  p_identificacion_civil_requerida boolean default false,
  p_codigo_interno_requerido boolean default false,
  p_tipos_identificacion_permitidos text[] default array['identidad', 'pasaporte', 'otro']::text[]
)
...
begin
  if public.usuario_actual_id() is null
     or not public.usuario_tiene_permiso_actual('configuracion.instituciones.editar', p_institucion_id) then
    raise exception 'Permiso denegado.' using errcode = '42501';
  end if;
  if btrim(coalesce(p_nombre, '')) = '' then
    raise exception 'El nombre del centro educativo es obligatorio.' using errcode = '22023';
  ...
  update public.instituciones set
    nombre = btrim(p_nombre), nombre_corto = nullif(btrim(p_nombre_corto), ''),
    direccion = nullif(btrim(p_direccion), ''), telefono = nullif(btrim(p_telefono), ''),
    correo = nullif(lower(btrim(p_correo)), ''), logo_url = nullif(btrim(p_logo_url), ''),
    updated_at = now() where id = p_institucion_id;
  insert into public.configuracion_identificadores (
    institucion_id, rne_requerido, identificacion_civil_requerida,
    codigo_interno_requerido, tipos_identificacion_permitidos)
  values (p_institucion_id, p_rne_requerido, p_identificacion_civil_requerida,
    p_codigo_interno_requerido, v_tipos)
  on conflict (institucion_id) do update set
    rne_requerido = excluded.rne_requerido,
...
-- Bloque 016: grados y jornadas son catalogos globales; secciones se contextualizan
-- por institucion y ciclo. No existe DELETE fisico mediante estas RPC.
create unique index ux_grados_nombre_normalizado on public.grados(lower(btrim(nombre)));
create unique index ux_jornadas_nombre_normalizado on public.jornadas(lower(btrim(nombre)));

insert into public.permisos(codigo,modulo,nombre) values
 ('configuracion.grados.ver','configuracion','Ver grados'),('configuracion.grados.crear','configuracion','Crear grados'),('configuracion.grados.editar','configuracion','Editar grados'),('configuracion.grados.desactivar','configuracion','Desactivar grados'),
 ('configuracion.jornadas.ver','configuracion','Ver jornadas'),('configuracion.jornadas.crear','configuracion','Crear jornadas'),('configuracion.jornadas.editar','configuracion','Editar jornadas'),('configuracion.jornadas.desactivar','configuracion','Desactivar jornadas'),
 ('configuracion.secciones.ver','configuracion','Ver secciones'),('configuracion.secciones.crear','configuracion','Crear secciones'),('configuracion.secciones.editar','configuracion','Editar secciones'),('configuracion.secciones.desactivar','configuracion','Desactivar secciones');
insert into public.roles_permisos(rol_id,permiso_id)
...
create function public.rpc_listar_grados(p_institucion_id uuid default null) returns table(id uuid,nombre text,orden integer,activo boolean) language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare v uuid;begin v:=public.resolver_institucion_operacion(p_institucion_id);if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual('configuracion.grados.ver',v) then raise exception 'Permiso denegado.' using errcode='42501';end if;return query select g.id,g.nombre,g.orden,g.activo from public.grados g order by g.orden,g.nombre;end$$;
create function public.rpc_crear_grado(p_nombre text,p_orden integer,p_institucion_id uuid default null) returns uuid language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare v uuid;x uuid;begin v:=public.resolver_institucion_operacion(p_institucion_id);if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual('configuracion.grados.crear',v) then raise exception 'Permiso denegado.' using errcode='42501';end if;if btrim(coalesce(p_nombre,''))='' or p_orden is null or p_orden<0 then raise exception 'Nombre u orden de grado no valido.' using errcode='22023';end if;insert into public.grados(nombre,orden) values(btrim(p_nombre),p_orden) returning id into x;return x;end$$;
create function public.rpc_actualizar_grado(p_grado_id uuid,p_nombre text,p_orden integer,p_institucion_id uuid default null) returns void language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare v uuid;begin v:=public.resolver_institucion_operacion(p_institucion_id);if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual('configuracion.grados.editar',v) then raise exception 'Permiso denegado.' using errcode='42501';end if;if btrim(coalesce(p_nombre,''))='' or p_orden is null or p_orden<0 then raise exception 'Nombre u orden de grado no valido.' using errcode='22023';end if;update public.grados set nombre=btrim(p_nombre),orden=p_orden,updated_at=now() where id=p_grado_id;if not found then raise exception 'El grado no existe.' using errcode='P0002';end if;end$$;
create function public.rpc_cambiar_estado_grado(p_grado_id uuid,p_activo boolean,p_institucion_id uuid default null) returns void language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare v uuid;begin v:=public.resolver_institucion_operacion(p_institucion_id);if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual(case when p_activo then 'configuracion.grados.editar' else 'configuracion.grados.desactivar' end,v) then raise exception 'Permiso denegado.' using errcode='42501';end if;update public.grados set activo=p_activo,updated_at=now() where id=p_grado_id;if not found then raise exception 'El grado no existe.' using errcode='P0002';end if;end$$;
create function public.rpc_desactivar_grado(p_grado_id uuid,p_institucion_id uuid default null) returns void language sql security definer set search_path=pg_catalog,public,pg_temp as $$select public.rpc_cambiar_estado_grado(p_grado_id,false,p_institucion_id)$$;
create function public.rpc_reactivar_grado(p_grado_id uuid,p_institucion_id uuid default null) returns void language sql security definer set search_path=pg_catalog,public,pg_temp as $$select public.rpc_cambiar_estado_grado(p_grado_id,true,p_institucion_id)$$;

create function public.rpc_listar_jornadas(p_institucion_id uuid default null) returns table(id uuid,nombre text,activo boolean) language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare v uuid;begin v:=public.resolver_institucion_operacion(p_institucion_id);if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual('configuracion.jornadas.ver',v) then raise exception 'Permiso denegado.' using errcode='42501';end if;return query select j.id,j.nombre,j.activo from public.jornadas j order by j.nombre;end$$;
create function public.rpc_crear_jornada(p_nombre text,p_institucion_id uuid default null) returns uuid language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare v uuid;x uuid;begin v:=public.resolver_institucion_operacion(p_institucion_id);if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual('configuracion.jornadas.crear',v) then raise exception 'Permiso denegado.' using errcode='42501';end if;if btrim(coalesce(p_nombre,''))='' then raise exception 'El nombre de la jornada es obligatorio.' using errcode='22023';end if;insert into public.jornadas(nombre) values(btrim(p_nombre)) returning id into x;return x;end$$;
create function public.rpc_actualizar_jornada(p_jornada_id uuid,p_nombre text,p_institucion_id uuid default null) returns void language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare v uuid;begin v:=public.resolver_institucion_operacion(p_institucion_id);if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual('configuracion.jornadas.editar',v) then raise exception 'Permiso denegado.' using errcode='42501';end if;if btrim(coalesce(p_nombre,''))='' then raise exception 'El nombre de la jornada es obligatorio.' using errcode='22023';end if;update public.jornadas set nombre=btrim(p_nombre),updated_at=now() where id=p_jornada_id;if not found then raise exception 'La jornada no existe.' using errcode='P0002';end if;end$$;
create function public.rpc_cambiar_estado_jornada(p_jornada_id uuid,p_activo boolean,p_institucion_id uuid default null) returns void language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare v uuid;begin v:=public.resolver_institucion_operacion(p_institucion_id);if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual(case when p_activo then 'configuracion.jornadas.editar' else 'configuracion.jornadas.desactivar' end,v) then raise exception 'Permiso denegado.' using errcode='42501';end if;update public.jornadas set activo=p_activo,updated_at=now() where id=p_jornada_id;if not found then raise exception 'La jornada no existe.' using errcode='P0002';end if;end$$;
create function public.rpc_desactivar_jornada(p_jornada_id uuid,p_institucion_id uuid default null) returns void language sql security definer set search_path=pg_catalog,public,pg_temp as $$select public.rpc_cambiar_estado_jornada(p_jornada_id,false,p_institucion_id)$$;
create function public.rpc_reactivar_jornada(p_jornada_id uuid,p_institucion_id uuid default null) returns void language sql security definer set search_path=pg_catalog,public,pg_temp as $$select public.rpc_cambiar_estado_jornada(p_jornada_id,true,p_institucion_id)$$;

drop function if exists public.rpc_desactivar_seccion(uuid,text);
create function public.rpc_actualizar_seccion(p_seccion_id uuid,p_ciclo_id uuid,p_grado_id uuid,p_jornada_id uuid,p_nombre text,p_cupo integer,p_institucion_id uuid default null) returns void language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare s public.secciones%rowtype;v uuid;begin select * into s from public.secciones where id=p_seccion_id for update;if not found then raise exception 'La seccion no existe.' using errcode='P0002';end if;v:=public.resolver_institucion_operacion(p_institucion_id);if s.institucion_id<>v then raise exception 'La seccion no pertenece a la institucion actual.' using errcode='P0002';end if;if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual('configuracion.secciones.editar',v) then raise exception 'Permiso denegado.' using errcode='42501';end if;if exists(select 1 from public.matriculas where seccion_id=s.id) and (p_ciclo_id is distinct from s.ciclo_id or p_grado_id is distinct from s.grado_id or p_jornada_id is distinct from s.jornada_id) then raise exception 'Una seccion con matriculas no puede cambiar de ciclo, grado ni jornada.' using errcode='23514';end if;if not exists(select 1 from public.ciclos_escolares where id=p_ciclo_id and institucion_id=v and activo) then raise exception 'El ciclo no pertenece a la institucion actual o esta inactivo.' using errcode='23503';end if;if not exists(select 1 from public.grados where id=p_grado_id and activo) then raise exception 'El grado no existe o esta inactivo.' using errcode='23503';end if;if p_jornada_id is not null and not exists(select 1 from public.jornadas where id=p_jornada_id and activo) then raise exception 'La jornada no existe o esta inactiva.' using errcode='23503';end if;if btrim(coalesce(p_nombre,''))='' or(p_cupo is not null and p_cupo<=0)then raise exception 'Nombre o cupo de seccion no valido.' using errcode='22023';end if;update public.secciones set ciclo_id=p_ciclo_id,grado_id=p_grado_id,jornada_id=p_jornada_id,nombre=btrim(p_nombre),cupo=p_cupo,updated_at=now() where id=s.id;end$$;
create function public.rpc_desactivar_seccion(p_seccion_id uuid,p_motivo text,p_institucion_id uuid default null) returns void language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare s public.secciones%rowtype;v uuid;begin select * into s from public.secciones where id=p_seccion_id for update;if not found then raise exception 'La seccion no existe.' using errcode='P0002';end if;v:=public.resolver_institucion_operacion(p_institucion_id);if s.institucion_id<>v then raise exception 'La seccion no pertenece a la institucion actual.' using errcode='P0002';end if;if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual('configuracion.secciones.desactivar',v) then raise exception 'Permiso denegado.' using errcode='42501';end if;if btrim(coalesce(p_motivo,''))='' then raise exception 'El motivo es obligatorio.' using errcode='22023';end if;update public.secciones set activo=false,fecha_desactivacion=now(),motivo_desactivacion=btrim(p_motivo),updated_at=now() where id=s.id;end$$;
create function public.rpc_reactivar_seccion(p_seccion_id uuid,p_institucion_id uuid default null) returns void language plpgsql security definer set search_path=pg_catalog,public,pg_temp as $$declare s public.secciones%rowtype;v uuid;begin select * into s from public.secciones where id=p_seccion_id for update;if not found then raise exception 'La seccion no existe.' using errcode='P0002';end if;v:=public.resolver_institucion_operacion(p_institucion_id);if s.institucion_id<>v then raise exception 'La seccion no pertenece a la institucion actual.' using errcode='P0002';end if;if public.usuario_actual_id() is null or not public.usuario_tiene_permiso_actual('configuracion.secciones.editar',v) then raise exception 'Permiso denegado.' using errcode='42501';end if;if not exists(select 1 from public.ciclos_escolares where id=s.ciclo_id and institucion_id=v and activo)or not exists(select 1 from public.grados where id=s.grado_id and activo)or(s.jornada_id is not null and not exists(select 1 from public.jornadas where id=s.jornada_id and activo))then raise exception 'La estructura academica de la seccion no existe o esta inactiva.' using errcode='23503';end if;update public.secciones set activo=true,fecha_desactivacion=null,motivo_desactivacion=null,updated_at=now() where id=s.id;end$$;

revoke execute on function public.rpc_listar_grados(uuid),public.rpc_crear_grado(text,integer,uuid),public.rpc_actualizar_grado(uuid,text,integer,uuid),public.rpc_cambiar_estado_grado(uuid,boolean,uuid),public.rpc_desactivar_grado(uuid,uuid),public.rpc_reactivar_grado(uuid,uuid),public.rpc_listar_jornadas(uuid),public.rpc_crear_jornada(text,uuid),public.rpc_actualizar_jornada(uuid,text,uuid),public.rpc_cambiar_estado_jornada(uuid,boolean,uuid),public.rpc_desactivar_jornada(uuid,uuid),public.rpc_reactivar_jornada(uuid,uuid),public.rpc_actualizar_seccion(uuid,uuid,uuid,uuid,text,integer,uuid),public.rpc_desactivar_seccion(uuid,text,uuid),public.rpc_reactivar_seccion(uuid,uuid) from public,anon;
grant execute on function public.rpc_listar_grados(uuid),public.rpc_crear_grado(text,integer,uuid),public.rpc_actualizar_grado(uuid,text,integer,uuid),public.rpc_desactivar_grado(uuid,uuid),public.rpc_reactivar_grado(uuid,uuid),public.rpc_listar_jornadas(uuid),public.rpc_crear_jornada(text,uuid),public.rpc_actualizar_jornada(uuid,text,uuid),public.rpc_desactivar_jornada(uuid,uuid),public.rpc_reactivar_jornada(uuid,uuid),public.rpc_actualizar_seccion(uuid,uuid,uuid,uuid,text,integer,uuid),public.rpc_desactivar_seccion(uuid,text,uuid),public.rpc_reactivar_seccion(uuid,uuid) to authenticated;
