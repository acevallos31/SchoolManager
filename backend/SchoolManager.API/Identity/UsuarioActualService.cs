...
public sealed class UsuarioActualService(NpgsqlDataSource dataSource) : IUsuarioActualService
{
    ...
    )
    {
        await using var command = dataSource.CreateCommand("""
            select
              ...
              i.nombre,
              i.nombre_corto,
              coalesce(array_agg(distinct r.codigo order by r.codigo), '{}'::text[]) as roles,
              coalesce(
    ...
    {
        // La visibilidad global del platform_admin no depende del modo
        // mono/multi-institución de la implementación.
        await using var command = dataSource.CreateCommand("""
