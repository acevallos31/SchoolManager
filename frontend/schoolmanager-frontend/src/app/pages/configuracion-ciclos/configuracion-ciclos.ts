...
export class ConfiguracionCiclos implements OnInit {
  ...
  async guardarPeriodo(){const c=this.cicloSeleccionado;if(!c||this.guardando||!this.validarPeriodo())return;this.guardando=true;try{if(this.editandoPeriodo)await this.service.actualizarPeriodo(this.editandoPeriodo,this.periodoForm);else await this.service.crearPeriodo(c.id,this.periodoForm);this.mostrarPeriodo=false;this.exito('Período guardado correctamente.');await this.gestionarPeriodos(c);}catch(e){this.error(e);}finally{this.guardando=false;this.cdr.detectChanges();}}
  async cambiarPeriodo(p:PeriodoMatricula){try{if(p.activo)await this.service.desactivarPeriodo(p.cicloId,p.id);else await this.service.reactivarPeriodo(p.cicloId,p.id);if(this.cicloSeleccionado)await this.gestionarPeriodos(this.cicloSeleccionado);}catch(e){this.error(e);this.cdr.detectChanges();}}
  ...
  private validarPeriodo(){const p=this.periodoForm;if(!p.nombre.trim()||!p.fechaInicio||!p.fechaFin){this.error(new CicloEscolarError('El nombre y las fechas del período son obligatorios.','22023'));return false;}if(p.fechaInicio>p.fechaFin){this.error(new CicloEscolarError('La fecha de inicio del período no puede ser posterior a la fecha de fin.','22023'));return false;}return true;}
